...
LOGGER = logging.getLogger("gsconfig2.support")
...
class DimensionInfo:
    ...
    _lookup = (
        ...
        ('hours', 3600),
        ('days', 86400),
        ('months', 2628000000),  # this what geoserver computes for 1 month
        ('years', 31536000000)
    )
    ...
    def _multiplier(self, name):
        name = name.lower()
        found = [i[1] for i in self._lookup if i[0] == name]
        if not found:
            raise ValueError('invalid multipler: {}'.format(name))
        return found[0]
    ...
    def resolution_str(self):
        """
        if set, get the value of resolution as '<n> <period>s',
...
def md_dimension_info(name, node):
    ...
    child_text = lambda child_name: getattr(node.find(child_name),
                                            'text', None)
    resolution = child_text('resolution')
    default_value = node.find("defaultValue")
    if default_value is not None:
        strategy = default_value.find("strategy")
    else:
        strategy = None
...
class JDBCVirtualTable(object):
    def __init__(self, _name, _sql, _escapeSql, _geometry,
                 _keyColumn=None, _parameters=None):
        self.name = _name
        self.sql = _sql
...
def md_jdbc_virtual_table(key, node):
    ...
    for n_p in node.findall("parameter"):
        ...
        if p_defaultValue is not None:
            p_defaultValue = p_defaultValue.text
        p_regexpValidator = n_p.find("regexpValidator")
        if p_regexpValidator is not None:
            p_regexpValidator = p_regexpValidator.text
        parameters.append(JDBCVirtualTableParam(
            p_name,
            p_defaultValue,
            p_regexpValidator
        ))
...
class DynamicDefaultValuesConfiguration(object):
    def __init__(self, dimension, policy, default_value_expression):
        self.dimension = dimension
        self.policy = policy
        self.defaultValueExpression = default_value_expression
...
def md_dynamic_default_values_info(name, node):
    ...
    if configurations is not None:
        ...
        for n in node.findall("configuration"):
            ...
            default_value_expression = n.find("defaultValueExpression")
            if default_value_expression.text is not None:
                default_value_expression = default_value_expression.text
            else:
                default_value_expression = None
            conf = DynamicDefaultValuesConfiguration(
                dimension,
                policy,
...
def key_value_pairs(node):
    if node is not None:
        return dict((entry.attrib['key'], entry.text)
    ...
    if isinstance(metadata, DimensionInfo):
        ...
        if metadata.presentation is not None:
            ...
            if metadata.presentation not in accepted:
                msg = "metadata.presentation must be one of the following {}"
                raise ValueError(msg.format(accepted))
            else:
